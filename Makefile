...
# textlintのDockerイメージ名
TEXTLINT_DOCKER_IMAGE=textlint-local
...
# tex 用 Docker Command
DOCKER_CMD=docker run --rm $(UIDOPT) -w /workdir -v $(CURDIR):/workdir $(DOCKER_IMAGE)

# textlint 用
TEXTLINT_DOCKER_CMD=docker run --rm $(UIDOPT) -w /workdir -v $(CURDIR):/workdir $(TEXTLINT_DOCKER_IMAGE)


ifeq "$(USE_DOCKER)" "yes"
	LATEXMK_CMD=$(DOCKER_CMD) latexmk -pdflatex=platex
	WATCH_OPTION=-pvc -view=none
else
	LATEXMK_CMD=latexmk -pdflatex=platex
	WATCH_OPTION=-pvc
endif
...
.PHONY: build-textlint-docker
build-textlint-docker:
	docker build -f Dockerfile.textlint -t $(TEXTLINT_DOCKER_IMAGE) .

.PHONY: lint
lint: build-textlint-docker
	$(TEXTLINT_DOCKER_CMD) npm run lint
